...
void kvazaar_ip_sub::intra_get_angular_0()
{
    while(1)
    {
	// wait for start
	...
	wait(intra_get_angular_partial_done[0] & intra_get_angular_partial_done[1] &
	     intra_get_angular_partial_done[2]);
	intra_get_angular_done.notify(SC_ZERO_TIME);
...
void kvazaar_ip_sub::intra_get_angular_1()
{
	...
	intra_get_angular_partial_done[0].notify(SC_ZERO_TIME);
    }
}
void kvazaar_ip_sub::intra_get_angular_2()
{
	...
	intra_get_angular_partial_done[1].notify(SC_ZERO_TIME);
    }
}
...
void kvazaar_ip_sub::intra_get_angular_3()
{
	...
	intra_get_angular_partial_done[2].notify(SC_ZERO_TIME);
    }
}
...
void kvazaar_ip_sub::b_transport( tlm::tlm_generic_payload& trans, sc_time& delay )
{
	...
	printf("adr: %d, read: %d, len: %d", (unsigned int)adr, (cmd == tlm::TLM_READ_COMMAND), len);
	sc_stop();
	break;
    }
    }
